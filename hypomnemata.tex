...
immediate and visible area).  \I{up} demonstratives tend to mean the
future, \I{down} the past.  The \I{up/down} distinction might be
general (eye level), or geographic (up a hill).  Might shrink to micro
level (in a single, level house) based on orientation in the landscape
(door facing a river is \I{down}, say).  Possible to have separate
general and geographic sets.  These are mostly \textit{not} used to
indicate social status (but does occur rarely).
