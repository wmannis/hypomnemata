...
\begin{document}
\frenchspacing
\title{Hypomnemata Glossopoetica\footnote{This work, both the grammar text and the
  language, is licensed under the Creative Commons 4.0
  \href{https://creativecommons.org/licenses/by-nc-sa/4.0/}{BY-NC-SA}
  license.

  No generative AI has been used in the conceptualization,
  development, or creation of this work. 
}}
\author{Wm S. Annis}
\date{\today}
\maketitle
...
There is a tendency for languages to have more ways to mark goal
than source.
% https://www.cambridge.org/core/services/aop-cambridge-core/content/view/131DDD9831BBC7C28A60546F9FB77E19/S186698082400070Xa.pdf/div-class-title-the-goal-over-source-asymmetry-in-thai-and-korean-div.pdf
...
\subsection{Nominal Tense}
Can indicate either change of state (``my former house'') or discourse
reference, indicating when something was mentioned, seen, etc.,
generally as a shared past experience.  Some languages offer both
interpretations, or, if tense is graded, different combinations.
Mvskoke has 4 tense levels: one is not used on nouns at all, one is
only change of state, and the two (temporally) closest ones can be
either discourse or change of state.  There may be differences in
definiteness interpretations.

Past nominal tense can also be a decessive (``my late uncle'').

% "Nominal Tense as Modifiers of Resource Situations," Kimberly
% Johnson, http://www.lingref.com/cpp/wccfl/40/paper3706.pdf
...
\subsection{Celerative}
A small number of languages have morphology to encode speed (most
common in North America, Western Amazon, Sepik area of New Guinea).
This can show up as part of the semantics of associated motion
affixes, where it may indicate the speed of either the motion or the
action.  Purely celerative markers may encode ``quickly'' or
``slowly'' (though ``slowly'' is much rarer, and if it exists a
``quickly'' marker will exist).  A verb meaning ``run'' can be a
source for a pure celerative marker.

Celerative marking will slot into same place other adverbial affixes
go (such as ``in secret,'' ``again,'' ``wrongly,'' ``strongly,
much'').  However, these do not only occur in languages with richly
marked verbs (though most common there).

It is very rare not to have other ways to indicate speed, but ``it
goes quickly'' is the only option for ``it is fast'' in a small number
of languages.

Colexification of the idea ``quick, fast'' can also apply to
celerative marking (in order of documented frequency): ``soon, hurry,
early, immediately;'' and much rarer, ``strong, for a short time.''

% https://www.academia.edu/121835802/Celerative_the_encoding_of_speed_in_verbal_morphology

\subsection{Periodic Tense}
Quite rare verb marking which encodes specific time periods: ``at
night, in the morning, during the day,'' etc., or a season, ``in
winter,'' etc.  These markers co-occur with deictic tense.  Like
\I{am}, periodic tense marking is not the only way to mark a
particular time period.  In discourse, a full noun or adverb phrase
for time may come first, followed by use of periodic tense (again like
\I{am}), but both might occur in the same clause in some languages.
It is very rare for a single morpheme to encode both deictic and
periodic tense (such as a single past nocturnal affix, for example).
May be ``do for the whole night'' vs.\ ``do at night.''

Attested periods: ``at dawn, in the morning, at noon, during the day,
afternoon, evening, night; all day, all night; until dawn, until noon,
until afternoon, until nightfall; during winter, spring, summer,
fall.''

Some languages may have only a single periodic tense, others may be
elaborate (5--6).  1--4 such markers seems usual.  A marker for
\I{nocturnal} is very common, with \I{morning} next.  The seasonal
ones are least common (and particular to non-tropical zones).

% Periodic tense markers in the world's languages and their sources,
% Guillaume, Jackques, Apr 22,2023, https://doi.org/10.1515/flin-2023-2013
...
Long final vowels associated with length (of shapes, appearance);
monosyllables for punctual events, and closed syllables for
abruptness. 
% Articulatory features of phonemes pattern to iconic meanings:
% evidence from cross-linguistic ideophones 2021.
%
% https://www.bonniemclean.net/news/2025-02-27-anatomyiconicity/

In one cross-linguistic study, [r] was strongly associated with a
jagged line, and [l] with a straight line.  However, if the trill
(rather than flap) was the primary rhotic of the language, iconicity
was weakened.
% The alveolar trill is perceived as jagged/rough by speakers of
% different languages
...
towards an embedded clause can be marked by affixes, clitics,
particles, adverbs, and auxiliaries.  Jacaltec has two complementation
...
A few languages have two definite article types, with one marking
uniqueness (``The Nile'') and the other anaphoric.  A language with
only one article may show a similar distinction in various ways even
if not systematic (German ``zu dem'' vs.\ ``zu'').

% https://hal.archives-ouvertes.fr/hal-02881062/document
% https://www.glossa-journal.org/article/id/18227/
